...
#define GX_INFO_LENGTH_128_BYTE (128)					///< 128byte
//------------------------------------------------------------------------------
...
/// GX_FLAT FIELD CORRECTION PARAMETER struct
typedef struct GX_FLAT_FIELD_CORRECTION_PARAMETER
{
	GX_FLAT_FIELD_CORRECTION_PARAMETER()
		:pBrightBuf(0),
		pDarkBuf(0),
		emPixelFormat(GX_PIXEL_FORMAT_UNDEFINED),
		nImgWid(0),
		nImgHei(0),
		nFFCBlockSize(0),
		nFFCExpectedGray(0)
...
typedef struct GX_ENUM_ITEM_VALUE
{
	int64_t nCurValue;												///< Enumeration current values
	char    strCurSymbolic[GX_INFO_LENGTH_128_BYTE];				///<  The symbol name of the current value of the enumeration type
	int32_t reserved[4];											///<  reserve
}GX_ENUM_ITEM_VALUE;
