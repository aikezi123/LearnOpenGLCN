...
/**
\brief      注册重连事件回调函数
\attention  设备重连事件，开启自动重连功能后，设备重连成功时可以通过这个接口的回调方式传出，
			用户不需要获取事件的时候调用GXUnregisterDeviceReconnectCallback接口注销回调函数
\param [in]hDevice     设备句柄
\param [in]pUserParam  用户私有数据
\param [in]callBackFun 用户注册的回调函数
\return    GX_STATUS_SUCCESS             操作成功，没有发生错误
...
*/
//----------------------------------------------------------------------------------
GX_API GXRegisterDeviceReconnectCallback(GX_DEV_HANDLE hDevice,
	void* pUserParam,
	GXDeviceReconnectCallBack callBackFun);

//----------------------------------------------------------------------------------
/**
\brief      注销重连事件回调函数
\attention  GXRegisterDeviceReconnectCallback注册的回调不用时，关闭回调函数调用
\param [in]hDevice     设备句柄
\return    GX_STATUS_SUCCESS             操作成功，没有发生错误
		   GX_STATUS_NOT_INIT_API        没有调用GXInitLib初始化库
		   GX_STATUS_INVALID_HANDLE      用户传入非法的句柄
...
*/
//----------------------------------------------------------------------------------
GX_API GXUnregisterDeviceReconnectCallback(GX_DEV_HANDLE hDevice);

//----------------------------------------------------------------------------------
/**
\brief      注册断线事件回调函数
\attention  设备断线事件，开启自动重连后设备断线时会触发注册的回调函数，
			用户不需要获取事件的时候调用GXUnregisterDeviceDisconnectCallback接口注销回调函数
\param [in]hDevice     设备句柄
\param [in]pUserParam  用户私有数据
\param [in]callBackFun 用户注册的回调函数
\return    GX_STATUS_SUCCESS             操作成功，没有发生错误
...
*/
//----------------------------------------------------------------------------------
GX_API GXRegisterDeviceDisconnectCallback(GX_DEV_HANDLE hDevice,
	void* pUserParam,
	GXDeviceDisconnectCallBack callBackFun);

//----------------------------------------------------------------------------------
/**
\brief      注销断线事件回调函数
\attention  GXRegisterDeviceDisconnectCallback注册的回调不用时，关闭回调函数调用
\param [in]hDevice     设备句柄
\return    GX_STATUS_SUCCESS             操作成功，没有发生错误
		   GX_STATUS_NOT_INIT_API        没有调用GXInitLib初始化库
		   GX_STATUS_INVALID_HANDLE      用户传入非法的句柄
...
*/
//----------------------------------------------------------------------------------
GX_API GXUnregisterDeviceDisconnectCallback(GX_DEV_HANDLE hDevice);
...
GX_API GXDQBufTest(GX_DEV_HANDLE hDevice, void** ppBufferHandle, GX_FRAME_BUFFER **ppFrameBuffer, uint32_t nTimeOut);
//----------------------------------------------------------------------------------
/**
\Chinese：
