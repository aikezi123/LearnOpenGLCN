...
{
	char chFeatureName[GX_INFO_LENGTH_128_BYTE];	///> \Chinese 属性名称		\English Feature name
}GX_FEATURE_NAME;
...
typedef enum GX_FEATURE_ID
{
	...
	GX_DEV_BOOL_ENABLE_AUTOCONNECT			  = 2 | GX_FEATURE_BOOL | GX_FEATURE_LEVEL_DEV, ///< 开启关闭掉线重连功能
	GX_DEV_ENUM_DEVICE_CONNECT_STATUS		  = 3 | GX_FEATURE_ENUM | GX_FEATURE_LEVEL_DEV, ///< 掉线在线状态，参考GX_DEVICE_CONNECTION_STATUS_ENTRY
	GX_DEV_INT_AUTOCONNECT_COUNTS			  = 4 | GX_FEATURE_INT | GX_FEATURE_LEVEL_DEV, ///< 掉线重连次数
	GX_DEV_INT_AUTOCONNECT_TIMEOUT			  = 5 | GX_FEATURE_INT | GX_FEATURE_LEVEL_DEV, ///< 掉线重连超时时间
...
typedef enum GX_DEVICE_CONNECTION_STATUS_ENTRY
{
	GX_DEVICE_CONNECTION_STATUS_OFF = 0,              ///< 设备掉线
	GX_DEVICE_CONNECTION_STATUS_ON = 1,              ///< 设备在线
	GX_DEVICE_CONNECTION_STATUS_CONNECT = 2,              ///< 设备掉线重连中
}GX_DEVICE_CONNECTION_STATUS_ENTRY;
...
\param        [in]pFeatureNameList        节点名称存储位置
\param        [in|out]pCount              给定pFeatureNameList数组容量, 返回实际写入的个数
...
\brief      批量向用户指定的寄存器中写入用户给定的数据（仅限命令值为4字节长度的寄存器）
            ...
            可使用GXReadRemoteDevicePortStacked接口获取最新的寄存器值
\param		[in]hDevice         设备句柄
